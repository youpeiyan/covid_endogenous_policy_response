...
foreach var in sip soe scs lcc_sip lcc lcn lcn1 {
gen `var'_ori = `var'
}

** Run the basic log-log model regression
qui reghdfe logy lcc lcn lcn1 soe sip scs lcc_sip $xctrl1, absorb(geoid dbin) cluster(stateid)
predict logy1
...
** counterfactural: no county case report & policy interaction
replace lcc = 0
replace lcc_sip = 0
...
graph export $path/results/figures/fig5a.png, replace 
graph export $path/results/figures/fig5a.pdf, replace 
...
twoway line y1 y2 y6 y5 y3 y7 y4 sipday if sipday <=20 & sipday >= -40, ///
		...
		lp(solid dash dash dash dash dash dash) ///
			...
			lab(5 "no case reports") lab(6 "no county-case reports") ///
			lab(7 "no interaction") ) ///
			graphregion(color(white)) bgcolor(white) 
graph export $path/results/figures/fig5.png, replace 
*/
